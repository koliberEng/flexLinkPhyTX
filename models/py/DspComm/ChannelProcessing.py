...
# Notes:      This script supports a number of tasks including the following functions:
#             1. ComputeDopplerFrequency(VelocityKmh, CenterFrequencyHz)
#             2. CreateGaussianNoise()
#             3. AddGaussianNoise()
#             This script implements the following class(es)
#             1. CMultipathChannel
#                a. AddPath()
#                b. ResetMultipathChannel()
#                c. ComputeFrequencyResponse()
#                d. PlotFrequencyResponse()
#                e. ConvolveTimeDomainSignal()
#             2. CChannelModel
#                a. Initialize()
#                b. PCA_Computation()
#                c. CheckDmrsReConfiguration()
#                d. FrequencyResponseEstimator()

__title__     = "ChannelProcessing"
__author__    = "Andreas Schwarzinger"
__status__    = "preliminary - There is still a problem with PlotFrequencyResponse()"
__date__      = "Nov, 8th, 2022"
...
class CMultipathChannel():
    ...
    """
    def __init__(self):
        self.MultipathChannel    = {}   # self.MultipathChannel is a dictionary of lists
        self.NumberPaths         = 0
    ...
    def AddPath(self
        ...
        assert DelayInSec > -100e-6 and DelayInSec < 100e-6, 'The path delay of ' + str(DelayInSec) + \
                                                           ' seconds is outside of the valid range of +/- 50usec.'
        if (RoundToNearestSampleTime == True and SampleRate > 0):
            SampleStep = 1/SampleRate
            DelayInSec = SampleStep * round(DelayInSec / SampleStep)

        self.MultipathChannel[self.NumberPaths] = [Amplitude, DelayInSec, DopplerHz]
        self.NumberPaths += 1


    # -----------------------------------------------------------------
    # Function: ResetMultipathChannel()
    # -----------------------------------------------------------------
    ...
    # -----------------------------------------------------------------
    # Function: ComputeReFreqResponse()
    # -----------------------------------------------------------------
    def ComputeReFreqResponse(self
                           , ListResourceElements: list) -> np.ndarray:
        """
        ...
        """
        assert isinstance(ListResourceElements, list), 'The input argument must be a list'
        assert len(ListResourceElements) > 0,          'We must have some resource elements present'        
        assert isinstance(ListResourceElements[0], CResourceElement)
        ...
        for EntryIndex, ResourceElement in enumerate(ListResourceElements):
            ...
            for PathIndex in range(0, self.NumberPaths):
                [Amplitude, DelayInSec, DopplerHz] = self.MultipathChannel[PathIndex]  
    ...
    def ComputeFreqResponse(self
        ...
        """
        brief: ComputeFreqResponse computes the frequency response of the channel using the following parameters
        param: Coordinates -> This is a np.ndarray with 2 rows (row 0 - Time, row 1 - Frequency)
        """
        assert isinstance(Coordinates, np.ndarray), 'The Coordinate input argument features invalid type' 
        Rows, Columns  = Coordinates.shape
        assert Rows == 2,                           'The Coordinates np.ndarray must have 2 rows.'
        assert Columns > 0,                         'The Coordinates np.ndarray must have more than 0 columns.'

        FreqResponse = np.zeros(Columns, dtype = np.complex64)

        for EntryIndex in range(0, Columns):
            TimeSec     = Coordinates[0, EntryIndex]
            FrequencyHz = Coordinates[1, EntryIndex]
            for PathIndex in range(0, self.NumberPaths):
                [Amplitude, DelayInSec, DopplerHz] = self.MultipathChannel[PathIndex]  
                FreqResponse[EntryIndex]   += Amplitude * np.exp(-1j*2*np.pi*DelayInSec*FrequencyHz) * \
                                                          np.exp(1j*2*np.pi*TimeSec*DopplerHz)
    ...
    def PlotFrequencyResponse(self
        ...
        assert isinstance(frequencies, np.ndarray) or isinstance(frequencies, list)
    ...
    def ConvolveTimeDomainSignal(self
        ...
        """
        brief: This function convolves the input signal with the multipath channel.
        ...
        note1: The StartTime is a convenient addition. It is a time shift for the final output waveform
               For example, you can set it to the delay in seconds of the strongest path. This way your
               strongest path does not appear delayed in the output signal. However, all other paths are
               properly time delayed to the strongest path. I added this so that I can more easily find
               constellation positions after the Multipath channel was applied.  
        note2: We will use exterpolation in time by using the closes sample value
        note3: Ensure that the signal is reasonably oversampled as we are using linear interpolation
        """
        # Type checking
        assert isinstance(InputSignal, np.ndarray)
        assert np.issubdtype(InputSignal.dtype, np.complexfloating)
        ...
        OutputSignal = np.zeros(NumElements, dtype = InputSignal.dtype)
        ...
        for PathIndex in range(0, self.NumberPaths):
            ...
            DopplerTime     = np.arange(0, (NumElements - 0.1)*SamplePeriod, SamplePeriod)
            DopplerSignal   = Amplitude * np.exp(1j*2*np.pi*DopplerTime*DopplerHz)  
            SignalNoDelay   = InputSignal * DopplerSignal
            SignalWithDelay = np.interp(DopplerTime - DelayInSec + StartTime,  DopplerTime, SignalNoDelay)
...
if __name__ == "__main__":
    ...
    if Test == 1:
        ...
        Frequencies = np.arange(-10000000, 10000000, 100000, dtype = np.float64)
        Time        = 0 * np.ones(len(Frequencies), dtype = np.float64)
        Coordinates = np.vstack([Time, Frequencies])

        FrequencyResponse = MultipathChannel.ComputeFreqResponse(Coordinates)

        plt.figure(1)
        plt.plot(Frequencies, FrequencyResponse.real, 'r')
        plt.plot(Frequencies, FrequencyResponse.imag, 'b')
        plt.title('Frequency Response of Multipath Filter')
        plt.grid(True)
        plt.show()
    ...
    if Test == 2:
        ...
        # ----------------------------------------------
        MultipathChannel.ComputeReFreqResponse(DmrsList)
