...
#        We have extended the mapping by adding QAM256 and QAM1024
...
class CQamMappingIEEE():
    ...
    QAM256_LUT  = (1/np.sqrt(170))  * np.array([-15, -13, -9, -11, -1, -3, -7, -5, \
                                                 15,  13,  9,  11,  1,  3,  7,  5], dtype = np.float32)   
    QAM1024_LUT = (1/np.sqrt(682)) * np.array([-31, -29, -25, -27, -17, -19, -23, -21, -1, -3, -7, -5, -15, -13, -9 , -11, \
                                                 31,  29,  25,  27,  17,  19,  23,  21,  1,  3,  7,  5,  15,  13,  9 ,  11], dtype = np.float32)
    ...
    def Mapping(cls
        ...
        while InputBitIndex < NumInputBits:
            ...
            # Process 256QAM
            if(BitsPerSymbol == 8):
                QamTableOffsetI = 8 * InputBits[InputBitIndex + 0] + \
                                  4 * InputBits[InputBitIndex + 1] + \
                                  2 * InputBits[InputBitIndex + 2] + \
                                  1 * InputBits[InputBitIndex + 3]  
                QamTableOffsetQ = 8 * InputBits[InputBitIndex + 4] + \
                                  4 * InputBits[InputBitIndex + 5] + \
                                  2 * InputBits[InputBitIndex + 6] + \
                                  1 * InputBits[InputBitIndex + 7]
                I = cls.QAM256_LUT[QamTableOffsetI]
                Q = cls.QAM256_LUT[QamTableOffsetQ]
                QamSymbols[QamSymbolIndex] = np.complex256(I + 1j*Q)
                InputBitIndex  += 8
                QamSymbolIndex += 1
                continue
            ...
            # Process 1024QAM
            if(BitsPerSymbol == 10):
                QamTableOffsetI = 16 * InputBits[InputBitIndex + 0] + \
                                   8 * InputBits[InputBitIndex + 1] + \
                                   4 * InputBits[InputBitIndex + 2] + \
                                   2 * InputBits[InputBitIndex + 3] + \
                                   1 * InputBits[InputBitIndex + 4]  
                QamTableOffsetQ = 16 * InputBits[InputBitIndex + 5] + \
                                   8 * InputBits[InputBitIndex + 6] + \
                                   4 * InputBits[InputBitIndex + 7] + \
                                   2 * InputBits[InputBitIndex + 8] + \
                                   1 * InputBits[InputBitIndex + 9]
                I = cls.QAM1024_LUT[QamTableOffsetI]
                Q = cls.QAM1024_LUT[QamTableOffsetQ]
                QamSymbols[QamSymbolIndex] = np.complex1024(I + 1j*Q)
                InputBitIndex  += 10
                QamSymbolIndex += 1
                continue
    ...
    def HardDemapping(cls
        ...
        for Estimate in Estimate_x:
            ...
            if(BitsPerSymbol == 8):
                Real = Estimate.real
                Imag = Estimate.imag
                if(Real >  1.1): Real =  1.16
                if(Real < -1.1): Real = -1.16
                if(Imag >  1.1): Imag =  1.16
                if(Imag < -1.1): Imag = -1.16
                Real = 2 * (np.floor(Real * np.sqrt(170)/2) + 0.5)
                Imag = 2 * (np.floor(Imag * np.sqrt(170)/2) + 0.5)           
                OutputBits[BitIndex+0:BitIndex+3] = cls.QAM64_Table[Real]
                OutputBits[BitIndex+3:BitIndex+6] = cls.QAM64_Table[Imag]
                BitIndex += 6
            ...
            if(BitsPerSymbol == 6):
                Real = Estimate.real
                Imag = Estimate.imag
                if(Real >  1.1): Real =  1.2
                if(Real < -1.1): Real = -1.2
                if(Imag >  1.1): Imag =  1.2
                if(Imag < -1.1): Imag = -1.2
                Real = 2 * (np.floor(Real * np.sqrt(682)/2) + 0.5)
                Imag = 2 * (np.floor(Imag * np.sqrt(682)/2) + 0.5)           
                OutputBits[BitIndex+0:BitIndex+3] = cls.QAM64_Table[Real]
                OutputBits[BitIndex+3:BitIndex+6] = cls.QAM64_Table[Imag]
                BitIndex += 6
    ...
    def SoftDemapping(cls
        ...
        if False:
            if(BitsPerSymbol == 8):
                ...
                LlrBits[4] = cls.GetLLR([-3*C, -1*C,  3*C,  1*C], [-7*C, -5*C,  5*C,  7*C], y_Q, NoiseVariance, h)  
                LlrBits[5] = cls.GetLLR([-5*C, -3*C,  3*C,  5*C], [-7*C, -1*C,  1*C,  7*C], y_Q, NoiseVariance, h)  
                LlrBits[6] = cls.GetLLR([-3*C, -1*C,  3*C,  1*C], [-7*C, -5*C,  5*C,  7*C], y_Q, NoiseVariance, h)  
                LlrBits[7] = cls.GetLLR([-5*C, -3*C,  3*C,  5*C], [-7*C, -1*C,  1*C,  7*C], y_Q, NoiseVariance, h)  
...
if __name__ == "__main__":
    ...
    QAM16_LUT   =  np.array([-3, -1,  3,  1],                 dtype = np.float32)  # Table 18.10 in the specifications 

    Out         = np.matmul

    QAM256_LUT  =   np.array([-15, -13, -9, -11, -1, -3, -7, -5, \
                                                15,  13,  9,  11,  1,  3,  7,  5], dtype = np.float32)   
    QAM1024_LUT =   np.array([-31, -29, -25, -27, -17, -19, -23, -21, -1, -3, -7, -5, -15, -13, -9 , -11, \
                                                31,  29,  25,  27,  17,  19,  23,  21,  1,  3,  7,  5,  15,  13,  9 ,  11], dtype = np.float32)
    
    print(np.sum(QAM256_LUT  * QAM256_LUT))
    print(np.sum(QAM1024_LUT * QAM1024_LUT))
